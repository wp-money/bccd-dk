...
function themezee_load_custom_css() { 
	...
	if ( !isset($options['themeZee_color_activate']) or $options['themeZee_color_activate'] != 'true' ) {
		...
		wp_register_style('zeeNews_colorscheme', $stylesheet, array('zeeNews_stylesheet'));
		wp_enqueue_style( 'zeeNews_colorscheme');
	}
}
...
function themezee_load_web_fonts() { 
	...
	wp_register_style('themezee_default_font_two', 'http://fonts.googleapis.com/css?family=Francois+One');
